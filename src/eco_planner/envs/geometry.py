...
def world_points_to_local(
    points: np.ndarray, anchor_xy_m: np.ndarray, anchor_heading_rad: float
) -> np.ndarray:
...
def world_vectors_to_local(vectors: np.ndarray, anchor_heading_rad: float) -> np.ndarray:
    cosine = np.cos(anchor_heading_rad)
    sine = np.sin(anchor_heading_rad)
    rotation = np.array([[cosine, sine], [-sine, cosine]], dtype=np.float64)
...
def shortest_angle_delta(angle: np.ndarray) -> np.ndarray:
