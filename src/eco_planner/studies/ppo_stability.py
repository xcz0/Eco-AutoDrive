...
def _rank_validation_configs(
    ...
    for config_id, items in by_config.items():
        if len(items) != required_seed_count or any(item["state"] != "complete" for item in items):
            continue
        evaluations = [_evaluation_record(item) for item in items]
        if any(not value["passed"] for value in evaluations):
            continue
        worst_training = min(
            _finite_number(item, "minimum_episode_length_retention") for item in items
        )
        mean_route = sum(
            _finite_number(value, "route_progress_retention") for value in evaluations
        ) / len(evaluations)
        ranked.append((-worst_training, -mean_route, config_id))
    return [item[2] for item in sorted(ranked)]


def _evaluation_record(record: dict[str, object]) -> dict[str, object]:
...
def run_command(
    ...
    if command == "summarize":
        study_config = load_stability_config(study_path)
        payload = summarize_stage_a(_create_study(study_config, output_root), study_config)
