...
def run_command(
    command: Literal["search", "confirm", "held-out", "diagnostic"],
    study_path: Path,
    output_root: Path,
    ...
    *,
    ...
    if command != "diagnostic" and diagnostic is not None:
        raise ValueError("diagnostic selection requires the diagnostic operation")
    if command == "search":
