"""Typed manifest for the update effective-update region search."""
...
DEFAULT_STUDY = CONFIG_ROOT / "experiments" / "training" / "grid.yaml"
...
class TrainingGridConfig(_StrictModel):
    version: Literal[1]
...
def load_training_grid(path: Path) -> TrainingGridConfig:
    """Load one explicit effective-update study manifest."""

    return TrainingGridConfig.model_validate(load_resolved_yaml_mapping(path))
