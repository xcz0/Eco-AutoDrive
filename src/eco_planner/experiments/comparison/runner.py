...
def run_command(
    ...
    *,
    ...
    arm_config(protocol, arm)
    if action not in ("train", "evaluate"):
        raise ValueError(f"unsupported scalar operation: {action}")
    if action == "train" and (
        arm == protocol.frozen_arm or checkpoint_label is not None or checkpoint_path is not None
    ):
        raise ValueError("train requires a trained arm and does not accept checkpoints")
    ...
    if arm == protocol.frozen_arm:
        if checkpoint_label is not None or checkpoint_path is not None:
            raise ValueError("frozen arm does not accept checkpoints")
