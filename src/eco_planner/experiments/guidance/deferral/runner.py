"""Replanning-deferral trace without constructing a policy or optimizer."""
...
def run(config_path: Path, output_dir: Path, *, figures: bool = True) -> dict[str, Any]:
    study = DeferralConfig.model_validate(load_resolved_yaml_mapping(config_path))
    ...
    if job.resources is None:
        raise ValueError("deferral requires an explicit resource profile")
    if job.training.mode != "no_traffic" or not job.training.deterministic:
        raise ValueError("deferral requires deterministic no-traffic execution")
    ...
    if job.sampler.name != "ddim5" or job.sampler.ddim_stochasticity != 0:
        raise ValueError("deferral requires deterministic DDIM-5")
    ...
    write_json(
        output_dir / "runtime_metadata.json",
        {
            **collect_repository_metadata(REPOSITORY_ROOT),
            ...
            "cycles": study.total_window_steps,
            "execution_steps": study.execution_steps,
            "deterministic": True,
        },
    )
    ...
    try:
        ...
        for offset in range(0, len(scenarios), workers):
            for seed in study.noise_seeds:
                ...
                print(
                    f"Group {group + 1}: {[s.name for s in batch]}, noise={seed}",
                    flush=True,
                )
                episodes.extend(
                    collect_group(
                        ...
                        InterventionExecution(
                            ...
                            study.total_window_steps,
                            study.execution_steps,
                        ),
                        job.reward.energy,
                        output_dir,
                        group,
    ...
    return publish("guidance-deferral", output_dir, output_dir, figures=figures)


def save_decisions(
    episodes: list[dict[str, Any]],
    study: DeferralConfig,
    scenario_names: list[str],
    output_dir: Path,
) -> None:
    result = analyze_episodes(episodes, study, scenario_names)
    write_json(output_dir / "decisions.json", {"gate_c": result["gate_c"]})
