...
DEFAULT_CONFIG = ROOT / "configs" / "matrices" / "reward_sanity.yaml"
_SCORE_FIELDS = (
    "reward_total",
    "reward_ungated",
    "reward_gate",
...
class _InputOverrides(_StrictModel):
    ...
    velocity_xy_mps: list[StrictFloat] | None = Field(
        default=None, min_length=2, max_length=2
    )
...
def load_sanity_config(path: Path) -> _SanityConfig:
    ...
    return _SanityConfig.model_validate(raw)


def evaluate_sanity(config: _SanityConfig) -> dict[str, object]:
    reward_path = (ROOT / config.reward_config).resolve()
    reward_raw = OmegaConf.to_container(
        OmegaConf.load(reward_path), resolve=True, throw_on_missing=True
    )
...
def _reward_input(values: dict[str, object]) -> RewardStepInput:
    ...
    participants = tuple(
        TrafficParticipantState(
            ...
            kind=item.kind,
            position_xy_m=tuple(item.position_xy_m),
            heading_rad=item.heading_rad,
            velocity_xy_mps=tuple(item.velocity_xy_mps),
    ...
    return RewardStepInput(
        previous_position_xy_m=tuple(parsed.previous_position_xy_m),
        position_xy_m=tuple(parsed.position_xy_m),
        previous_velocity_xy_mps=tuple(parsed.previous_velocity_xy_mps),
        velocity_xy_mps=tuple(parsed.velocity_xy_mps),
        previous_acceleration_xy_mps2=tuple(parsed.previous_acceleration_xy_mps2),
        ...
        traffic_frame=TrafficFrame(
            simulator_step=1,
            ego_center_xy_m=tuple(parsed.position_xy_m),
