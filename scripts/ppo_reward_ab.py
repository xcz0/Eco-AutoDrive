...
DEFAULT_MATRIX = ROOT / "configs" / "matrices" / "ppo_reward_ab.yaml"
...
def load_ab_config(path: Path) -> PPORewardABConfig:
    ...
    return PPORewardABConfig.model_validate(raw)
...
def build_training_command(
    ...
    return [
        ...
        f"--config-name=jobs/training/{config.base_training_profile}",
        f"rl/reward={profile.reward_config}",
...
def run_ab(matrix_path: Path, output_root: Path) -> int:
    config = load_ab_config(matrix_path)
    output_root.mkdir(parents=True, exist_ok=False)
    OmegaConf.save(OmegaConf.load(matrix_path), output_root / "matrix_manifest.yaml", resolve=True)
        ...
        for replay_id in config.matched_training.replay_ids:
            ...
            for profile in config.profiles:
                ...
                command = build_training_command(
                    config, profile, training_seed, replay_id, run_dir
                )
    ...
    if failed:
        return 1
    from summarize_ppo_reward_ab import summarize_ab
...
def main() -> None:
    ...
    raise SystemExit(run_ab(args.matrix.resolve(), args.output_root.resolve()))
