...
from ppo_reward_ab import PPORewardABConfig, load_ab_config
...
def summarize_ab(root: Path) -> dict[str, object]:
    config = load_ab_config(root / "matrix_manifest.yaml")
    ...
    for seed in config.matched_training.training_seeds:
        for replay in config.matched_training.replay_ids:
            ...
            pairs.append(
                _pair_report(config, seed, replay, builtin, energy)
            )
    ...
    return {
        ...
        "review_questions": [
            ...
            "Did execution fuel-proxy intensity change, and is any decrease confounded by progress?",
...
def _load_run(path: Path, expected_profile: str) -> dict[str, object]:
    ...
    resolved = OmegaConf.to_container(
        OmegaConf.load(path / "resolved_config.yaml"), resolve=True, throw_on_missing=True
    )
    if not isinstance(resolved, dict):
        raise TypeError(f"{path} resolved config must be a mapping")
    updates = tuple(_load_update(path, index, expected_profile) for index in range(len(summary.updates)))
...
def _pair_checks(
    ...
    checks = [
        {"name": "resolved_configs_match_except_reward", "passed": _matched_configs(builtin, energy)},
        {"name": "resolved_config_matches_matrix", "passed": _resolved_matches_matrix(config, builtin)},
        {"name": "initial_policy_hash_matches", "passed": left.initial_policy_hash == right.initial_policy_hash},
        ...
        {"name": "pre_update_collection_is_exactly_paired", "passed": _initial_collection_equal(builtin, energy)},
    ]
    return checks
...
def _pair_report(
    ...
    changes = {
        "longitudinal_action_mean_delta": (
            energy_metrics["longitudinal_action_mean"]
            - builtin_metrics["longitudinal_action_mean"]
        ),
        ...
        "collision_count_delta": energy_metrics["collision_count"] - builtin_metrics["collision_count"],
...
def _resolved_matches_matrix(config: PPORewardABConfig, run: dict[str, object]) -> bool:
    ...
    if not isinstance(training, dict) or not isinstance(scenarios, list):
        return False
    expected_scenarios = [item.model_dump(mode="python") for item in config.matched_training.scenarios]
    return (
