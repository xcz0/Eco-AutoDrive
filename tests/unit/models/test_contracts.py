...
def test_observation_contract_returns_batch(
    ...
    assert validate_official_observation(stage0_observation, torch.device("cpu")) == 1
...
def test_observation_contract_rejects_missing_field(
    ...
    with pytest.raises(ValueError, match="missing required fields"):
        validate_official_observation(stage0_observation, torch.device("cpu"))
...
def test_observation_contract_rejects_empty_batch(
    ...
    with pytest.raises(ValueError, match="positive"):
        validate_official_observation(empty, torch.device("cpu"))
