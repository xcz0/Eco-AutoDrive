...
def test_sampler_config_parses_strict_profiles() -> None:
    ...
    assert sampler_report(ddim).implementation == "diffusers"
...
def test_dpm_config_rejects_irrelevant_math_fields() -> None:
    with pytest.raises(ValueError, match="unexpected"):
        parse_sampler_config(
            OmegaConf.create({"name": "dpm10", "implementation": "diffusers", "num_steps": 10})
        )


def test_sampler_config_rejects_removed_legacy_backend() -> None:
    with pytest.raises(ValueError, match="diffusers"):
        parse_sampler_config(_ddim_config(implementation="legacy"))  # type: ignore[arg-type]
