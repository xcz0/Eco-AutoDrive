...
from eco_planner.evaluation.runtime import configure_job_execution
...
def test_cpu_parallel_execution_applies_explicit_thread_budget(
    ...
    applied: list[int] = []
    monkeypatch.setattr("eco_planner.evaluation.runtime.os.cpu_count", lambda: 8)
...
def test_cpu_parallel_execution_rejects_oversubscribed_threads(
    ...
    monkeypatch.setattr("eco_planner.evaluation.runtime.os.cpu_count", lambda: 4)
    monkeypatch.setattr(torch.cuda, "is_available", lambda: False)
