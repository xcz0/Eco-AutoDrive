...
def test_map_adapter_builds_official_raw_array_contract(
    ...
    result = adapter.build_arrays(env)

    assert result["lanes"].shape == (70, 20, 12)
    assert result["lanes_speed_limit"].shape == (70, 1)
    assert result["lanes_has_speed_limit"].shape == (70, 1)
    assert result["route_lanes"].shape == (25, 20, 12)
    assert result["route_lanes_speed_limit"].shape == (25, 1)
    assert result["route_lanes_has_speed_limit"].shape == (25, 1)
    assert result["lanes"].dtype == np.float32
    assert result["lanes_has_speed_limit"].dtype == np.bool_
    assert result["lanes_speed_limit"][0, 0] == pytest.approx(10.0)
    assert result["lanes_has_speed_limit"][0, 0]

    first_lane = result["lanes"][0]
    np.testing.assert_array_equal(first_lane[0, :2], [0.0, 0.0])
    np.testing.assert_array_equal(first_lane[0, 4:6], [0.0, 2.0])
    np.testing.assert_array_equal(first_lane[0, 6:8], [0.0, -2.0])
    np.testing.assert_array_equal(first_lane[0, 8:12], [0.0, 0.0, 0.0, 1.0])
    np.testing.assert_array_equal(first_lane[-1, 2:4], np.zeros(2))
    assert np.count_nonzero(result["lanes"][2:]) == 0
    np.testing.assert_array_equal(result["route_lanes"][:2], result["lanes"][:2])
...
def test_map_adapter_marks_missing_speed_limit_explicitly(
    ...
    result = MetaDriveMapAdapter(official_model_config, 100.0).build_arrays(env)

    assert result["lanes_speed_limit"][0, 0] == 0.0
    assert not result["lanes_has_speed_limit"][0, 0]
...
def test_map_adapter_preserves_mixed_explicit_speed_limit_encoding(
    ...
    np.testing.assert_allclose(
        result["lanes_speed_limit"][:2, 0], [50.0 / 3.6, 20.0 / 3.6], atol=1e-6
    )
    assert result["lanes_has_speed_limit"][:2, 0].tolist() == [True, True]
...
def test_map_adapter_filters_radius_and_keeps_connected_route(
    ...
    assert np.count_nonzero(result["lanes"][:, 0, 11]) == 3
    assert np.count_nonzero(result["route_lanes"][:, 0, 11]) == 2
...
def test_map_adapter_truncates_lane_and_route_capacity(
    ...
    assert np.count_nonzero(result["lanes"][:, 0, 11]) == 70
    assert np.count_nonzero(result["route_lanes"][:, 0, 11]) == 25
...
def test_map_adapter_rejects_missing_navigation(
    ...
    with pytest.raises(RuntimeError, match="navigation"):
        MetaDriveMapAdapter(official_model_config, 100.0).build_arrays(env)
