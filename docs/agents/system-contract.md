...
`MetaDriveEnvSlot.step()` 调用 trajectory executor：executor 使用 canonical local-to-world conversion，逐个请求 backend 的 0.1 s transition；stateful transition extractor 从连续 ego state、目标点、交通快照、route/lane 与 typed MetaDrive termination outcome 生成 objective-neutral `TransitionMetrics`。executor 在 terminal/truncation 时停止，并构造 domain-owned、不可变的 `TrajectoryExecutionRecord`；该 record 只保存 start/target/actual trajectory、traffic、route 与 termination 等执行事实，不保存 reward、误差、distance、energy 或 metrics。slot 从 builtin reward 或注入的 objective 生成独立 `EnvSlotStep`，其中携带总量和逐子步 reward、dense reward、metrics、终止状态与 execution record；backend 不构造这些上层对象，调用方也不从原始 `info` 二次解析。RL 从 step metrics 计算 reward audit；evaluation recorder 从同一 step result 映射既有 trace 字段，不二次实现 reward、误差或 fuel 公式。数组的 shape/dtype 由固定容量执行缓冲、jaxtyping 接口契约和 producer 测试保证。

## 能耗记录

* 每种能耗指标使用独立名称、单位和累计边界。`envs.domain.metrics` 拥有 `EnergyTrace`、`EnergyMetrics` 与 provider protocol；provider 接收有限、非负且时间严格递增的 `EnergyTrace(time_s[N+1], speed_mps[N+1], step_distance_m[N])`，返回带 metric 名称、实际距离、可选 J 和可选 mL 的 `EnergyMetrics`；Wh 与各 per-km quantity 只由该统一结果派生，零距离时 per-km 为未定义。
