...
训练 reward 的唯一已实现 profile 是 `extra="forbid"` 的严格 `plannerrft_energy_v1`。配置解析、component 映射、safety gate、聚合和 `RewardResult` 全部归 `eco_planner.rl.reward`；`envs`、MetaDrive worker 和 TorchRL adapter 不 import 或执行 reward policy。TTC corridor/threshold 解释也归 reward 层，`envs.domain.metrics` 只保留客观运动、执行误差和能耗量。safety gate 为 collision、当前 `out_of_road` 与当前 route/reference lane 前向切线的 wrong-direction 判定之积；未 gated components 为 TTC、非负 route-lane 纵向 progress、实际执行 velocity/acceleration/yaw-rate 得到的 comfort、当前 lane speed limit 下的 speed，以及 energy provider 产生的 fuel proxy metric。全部阈值、权重、归一化尺度和 corridor margin 来自 resolved reward profile；PPO/GAE 只接收 `RewardResult.total`。
...
`MetaDriveEnvSlot.step()` 调用 trajectory executor：executor 使用 canonical local-to-world conversion，逐个请求 backend 的 0.1 s transition；stateful transition extractor 从连续 ego state、目标点、交通快照、route/lane 与 typed MetaDrive termination outcome 生成 objective-neutral `TransitionMetrics`。executor 在 terminal/truncation 时停止，并返回 domain-owned、不可变的单一 `TrajectoryExecutionResult(execution, metrics, terminated, truncated)`；`TrajectoryExecutionRecord` 只保存 start/target/actual trajectory、traffic、route 与 termination 等执行事实。backend、executor、slot、worker 都不携带 builtin reward、训练 reward callback 或 reward-specific audit。RL collector 从结果的单步 metrics 生成 `RewardResult`；evaluation recorder 从同一结果映射客观 trace 字段，不保存 reward。数组的 shape/dtype 由固定容量执行缓冲、jaxtyping 接口契约和 producer 测试保证。
