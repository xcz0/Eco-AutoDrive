...
`MetaDriveEnvSlot.step()` 调用 trajectory executor：executor 使用 canonical local-to-world conversion，逐个请求 backend 的 0.1 s transition；stateful transition extractor 从连续 ego state、目标点、交通快照、route/lane 与 typed MetaDrive termination outcome 生成 objective-neutral `TransitionMetrics`。executor 在 terminal/truncation 时停止并生成 domain-owned `TrajectoryExecutionResult(execution, metrics, terminated, truncated)`；slot 提交 traffic frames、构造下一 `EnvSlotState`，再作为原子 `EnvSlotStep` 返回。`TrajectoryExecutionRecord` 只保存 start/target/actual trajectory、traffic、route 与 termination 等执行事实。backend、executor、slot、worker 都不携带 builtin reward、训练 reward callback 或 reward-specific audit。RL collector 对结果的每个 substep `TransitionMetrics` 求 `RewardResult`，再按 `reward.aggregate_transition_reward` 聚合为该 transition 的一个 `RewardResult`，并把逐 substep 的 component score、safety gate 与重校准输入持久化到 CPU audit，使离线 reweight/rescore/verify 逐 substep 复用同一 `reward.aggregate_substep_rewards` 归约；evaluation recorder 从同一结果映射客观 trace 字段，不保存 reward。数组的 shape/dtype 由固定容量执行缓冲、jaxtyping 接口契约和 producer 测试保证。
...
* 结果必须明确限定在运动学执行条件；全部 PlannerRFT reward profile（`plannerrft_energy_v1`、`plannerrft_no_energy_v1`、`plannerrft_no_energy_calibrated_v1`、`plannerrft_energy_band_lam{1,2,4,8,64}_v1`）都是 smoke-only PlannerRFT-style MetaDrive adaptation，不代表 PlannerRFT/nuPlan scorer parity、真实车辆舒适性或已验证的节能目标。
...
每个回合至少保存 `summary.json` 和 `trace.npz`；开启视频时保存闭环 GIF。trace 必须包含 raw observation、初始噪声、完整联合预测、规划锚点、目标与实际状态、逐点误差、逐子步 wrapped route heading error、逐子步 domain 行为事实（speed、stopped、wrong_direction）、逐子步 native MetaDrive energy、execution-recomputed fuel proxy、实际 distance 和终止标志；交通回合还保存预热、对象 ID、交通数量、最近交通距离和历史有效性。

`envs.domain.transition.TransitionMetrics` 是所有 training/evaluation 消费者共享的客观事实边界：它在 execution boundary 单点推导 motion、behavior 与 energy quantity，固定单位、坐标系与符号。`route_heading_error_rad` 是实际 heading 相对 route forward tangent 的 wrapped absolute error（rad，非负），`wrong_direction` 为 `route_heading_error_rad > WRONG_DIRECTION_MAX_HEADING_ERROR_RAD`（当前 π/2）；`stopped` 为 `speed_mps < STOPPED_SPEED_THRESHOLD_MPS`（当前 0.1 m/s）；`collision` 是 `any(crash_*)` 的 canonical 派生；`speed_mps` 是 velocity 的欧氏范数，`step_distance_m` 是相邻实际 center 的几何距离，`position_error_m`/`heading_error_rad` 是相对目标轨迹的执行误差。测试/实验可聚合不同窗口，但不得同名不同义，也不得在 reward、rl 或 evaluation 重新推导这些单步量或行为判定。

`evaluation.artifacts.summary.compute_episode_metrics` 是通用 closed-loop metric 的唯一计算路径：它从完整 execution trace 与最终 typed execution record 生成一个 `evaluation_episode` 聚合单位的 `EpisodeMetrics`。其中 distance 是 initial state 到每个实际 executed state 的几何路径长度；mean speed 与 stopped fraction 分别聚合 trace 中逐子步 domain `executed_speed_mps` 与 `executed_stopped`；route completion 是最终 execution record 的值；arrival、collision 与 out-of-road 取自最终 terminal execution record（collision 使用 record 的 canonical `any(crash_*)` 派生）；total energy、energy distance 与 energy intensity 只从 execution-recomputed fuel-proxy trace 流聚合。wrong-direction 指标直接聚合 trace 的逐子步 domain `executed_wrong_direction` bool：任一子步为 true 即 `wrong_direction=true`，`wrong_direction_fraction` 为该子步比例；trace 仍保留 `executed_route_heading_errors_rad` 供诊断，但不再用它重算阈值。该指标只记录，不改变 termination。summary、matrix 与实验报告消费这些 typed episode/training summaries，不得另从 trace array 或 resolved config 重算同名通用指标。matrix report 的 episode 行包含 `wrong_direction`，scenario 统计包含 `wrong_direction_rate`。
