...
| G-02 | reference trajectory 如何生成？ | 当前只有 DPM baseline；候选训练需要 DDIM 与冻结 reference | 固定/随机 reference sampler；每周期共享或独立噪声 | sampler、seed、刷新频率、checkpoint ownership 的书面定义和测试 |
| G-03 | orthogonal guidance 的离散定义和 neutral action 是什么？ | 模型输出 ego rear-axle local `10 Hz [80,4]`；论文纵向 Eq. 3 在 zero scale 处有公开歧义；不得投影/平滑 | 忠实公式并限定结论；经标注的 reference-centered 修正；其他显式定义 | 公式、单位、zero-scale 语义、端点/零速/退化轨迹错误边界、rigid-transform 测试 |
...
| G-12 | guidance 梯度在哪个空间、作用于谁？ | 当前模型在 normalized joint state 去噪，论文未公开注入系数/空间/是否 ego-only | physical ego energy 经 normalizer 链式求导；normalized joint energy；其他显式边界 | 梯度对象、每步系数、normalization Jacobian、邻车通道和有限性测试 |
