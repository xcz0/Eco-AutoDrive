...
**更新日期**：2026-08-04  
**当前里程碑**：阶段 0 完成；阶段 1 仅完成无交通短程闭环的接口修正与本机验证。
...
- Ubuntu 22.04 / CUDA 12.4 Docker 环境和 Linux/amd64 离线包脚本已建立；
...
- Ubuntu/Docker/CUDA 下的依赖、完整测试和官方权重闭环；
...
1. 在 Ubuntu Docker/CUDA 环境运行相关测试和现有 2 s 短闭环，确认平台一致性；
2. 运行预先定义的 `S`/`SC`、多噪声 seed、20 s 无交通矩阵，并完整登记实验元数据；
3. 完成上游逐层数值对照，分离剩余的模型移植差异与 MetaDrive 输入域差异；
4. 只有无交通基线稳定且结论可复现后，再实现有交通 21 帧历史适配；
