...
| [E-043](records/e-043-issue98-task-a-execution-horizon.md) | 2026-09-16 | 正式配对闭环人工干预 | Issue #98 Task A：冻结 planner、固定 2 s 窗口，execution horizon k∈{1,2,5,10,20} waypoints/plan，S/SC×seeds 0–7×3 noise×5 arms | `g_lon`→executed speed 由 k=1 负（10/16）转为 k≥2 正（16/16），与 planner full-trajectory 正响应一致，支持归因 A；73/1200 k=2 负 guidance off-route，机器 gate `safety_or_proxy_failure`；Task B–E 未执行 |
| [E-044](records/e-044-issue98-task-b-guidance-decomposition.md) | 2026-09-17 | 正式配对闭环人工干预 | Issue #98 Task B：E-041 冻结的 r0/lon/lat/joint 四常量 guidance 臂（原生 [lateral, longitudinal]），E-040 held-out 协议（S/SC seeds 16–23、horizon 300、runtime seed 760025、ddim5）冻结 planner 干预 | energy 双 seed `longitudinal-dominated`；speed seed0 `nonlinear-interaction`（lon/joint=0.72 阈值边界）、seed1 `longitudinal-dominated`、overall `mixed-across-seeds`；joint 方向复现 E-040（positive）；确认较大迁移来自纵向分量，不支持横纵耦合为主因；SC 49/64 off-route 保留；Task C–E 未执行 |
| [E-045](records/e-045-issue98-task-c-replanning-deferral.md) | 2026-09-17 | 正式配对闭环人工干预 | Issue #98 Task C：冻结 planner、baseline 0.1 s receding horizon，连续 20 replanning cycle 的逐 waypoint matched guidance effect（S/SC×seeds 0–7×3 noise×5 arms，2 s 窗口，execution_steps=1） | Gate C `repeated_deferral`（13/16 场景）：首点效应多数 ≤0、8 s full-horizon 多数 >0、正 response 越过执行 prefix、zero-crossing step 稳定在 0.2 s；cycle-0 与 E-043 首规划一致；安全 clean；Task D–E 未执行 |
...
| [E-049](records/e-049-issue83-task-1b-effective-update-transfer.md) | 2026-09-24 | 正式单臂训练 transfer gate | Issue #83 Task 1B：calibrated R0 + E-039 candidate PPO（lr=1.5e-4/epochs=1/mgn=0.5/target_kl=0.006）在 canonical k=5 下单臂 matched 50-update transfer run，复用冻结 Gate F（Gate T2） | Gate T2 FAILED（仅 c1）：median seeded-MC post-update KL 3.258e-7 < 1e-6；c2–c6 全通过（ratio 1.10e-3、probe RMS 0.0220、min α/β 1.93/1.99、0 collision/OOR、ep-length 8→8）、c7 未评估；相对 E-039/k=1（KL 1.16e-6 通过）actor 更新幅度整体略小、KL 降到门限下，critic loss/grad norm 显著增大；E-039 control 未 transfer 到 canonical cadence；顺带修复 canonical 多 substep reward component schema bug；Task 1C 未执行 |
